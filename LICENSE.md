# License

This repository uses a split license strategy.

Unless a directory has its own `LICENSE.md`, files are licensed under **Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**.

- License deed and legal code: https://creativecommons.org/licenses/by-sa/4.0/
- Attribution: **Mikhail Shakhnazarov**

## Directory license map

- `protocol/`: CC BY-SA 4.0 (protocol editions directory).
- `protocol/normative/`: CC BY-SA 4.0 (authoritative edition).
- `protocol/plain-language/`: CC BY-SA 4.0 (plain-language mirror; non-authoritative).
- `docs/`: CC BY-SA 4.0 OR CC0 1.0 (dual-licensed; see `docs/LICENSE.md`).
- `market-analysis-and-project-rationale/`: CC BY-SA 4.0 (reciprocal essays and background).
- `starter-kit/`: CC BY-SA 4.0 OR CC0 1.0 (dual-licensed; see `starter-kit/LICENSE.md`).
- `examples/`: CC BY-SA 4.0 OR CC0 1.0 (dual-licensed; see `examples/LICENSE.md`).
- `cc0/`: CC0 1.0 (copy/paste pack; see `cc0/LICENSE.md`).

## Notes

- CC BY-SA allows commercial use and redistribution, including in modified form, as long as attribution is provided and adaptations are shared under the same (or compatible) terms.
- This repository is text-first. If software code is added later, it may be licensed separately; the license for any code will be stated explicitly in the relevant directories.

Copyright (c) 2026 Mikhail Shakhnazarov
